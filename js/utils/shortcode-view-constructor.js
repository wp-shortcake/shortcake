	...
	View : {
		...
		fetch: function() {
			...
			wp.ajax.post( 'do_shortcode', {
				...
				self.shortcodeHTML = '<span class="shortcake-error">' + shortcodeUIData.strings.mce_view_error + '</span>';
				self.render( true );
			} );
		 ...
		 */
		setHtml: function( renderedShortcode ) {
			var MutationObserver = window.MutationObserver || window.WebKitMutationObserver || window.MozMutationObserver;

			if ( renderedShortcode.indexOf( '<script' ) === -1 ) {
				this.shortcodeHTML = renderedShortcode;
				this.render( true );
				return;
			}
