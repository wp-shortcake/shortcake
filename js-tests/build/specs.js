		...
		this.get( 'attrs' ).each( function( attr ) {
			...
			if ( attr.get( 'attr' ) === 'content' ) {
				...
				if ( ! isNaN( attr.get( 'attr' ) ) ) {
					...
					// console.log(attr, attr.get( 'value' ));
...
var shortcodeViewConstructor = {
	...
	getShortcodeModel: function( options ) {
		...
		shortcodeModel.get('attrs').each(
			function( attr ) {
				// console.log('in getShortcodeModel first', attr);
				...
				// console.log('in getShortcodeModel', options.shortcode.attrs.numeric);
				// if ( typeof options.shortcode.attrs.numeric !== 'undefined' ) {
	...
	parseShortcodeString: function( shortcodeString ) {
		...
		if ( matches[2] ) {
			...
			// console.log('before find match', attributeMatches.length);
